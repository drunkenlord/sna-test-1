	...
	request({uri: urlo, strictSSL: false},
		function (error, response, body) {
			...
			parser.parseString(body, function (err, jsonraw) {
				...
				fs.writeFileSync("/home/parallels/hapi_logs/" + cfile, JSON.stringify(jsonraw, null, 4));
...
function SSCWeb2HAPI(cb) {
	...
	if (fs.existsSync(cfile)) {
		...
		if (!force && age < 1) {
			...
			if (!fs.existsSync("/home/parallels/hapi_logs" + "/old/")) {
				fs.mkdirSync("/home/parallels/hapi_logs" + "/old/")
			}
			// Later versions of nodejs (8+) have copyFileSync.
			fs
				.createReadStream("/home/parallels/hapi_logs"+ "/" + cfile)
				.pipe(fs.createWriteStream("/home/parallels/hapi_logs" + "/old/" + cfilec))
				.on('finish', function () {
					getUrlo(cb);
